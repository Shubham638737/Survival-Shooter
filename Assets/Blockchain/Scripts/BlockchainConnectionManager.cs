        ...
        private async void ConnectExternalWallet()
        {
            try
            {
                ...
                if (wallet != null)
                {
                    ...
                    // Save in PlayerPrefs
                    PlayerPrefs.SetString("WALLET_ADDRESS", BlockchainManager.Instance.walletAddress);
                    PlayerPrefs.Save();

                    // Debug confirmation
                    if (PlayerPrefs.HasKey("WALLET_ADDRESS"))
                    {
                        Debug.Log("Wallet saved successfully: " + PlayerPrefs.GetString("WALLET_ADDRESS"));
                    }
                    else
                    {
                        Debug.LogError("❌ Wallet save failed!");
                    }
